...
## アプリケーションをより良いものに成長させるためのフロー

1. システムが燃える
  - 障害が発生する
    - ページが見られない、買えない、エラーが出る
1. エンドユーザーが障害に直面する
   1. エラーページが表示される、動かない。。。
   2. 購入完了しない。。。
   3. 見られない。。。
2. サポートセンターに問い合わせが発生する
   1. 障害が大規模だとサポセンがパンクする
3. サポートセンターが運用マニュアルに基づいて対応を行う
   1. 障害発生の場合はマニュアル化されてない
4. 開発会社（システム運用会社）に問い合わせを行う
   1. Backlogでチケットが発行される
   2. 最悪の場合は電話が鳴る（夜でも鳴る）
5. 開発会社が障害の原因調査する
   1. だいたい焦る
6. 障害の調査に応じて対応する
   ...
   - 対応時にミスが連鎖する事がある
7. 障害の報告を行う
   1. 監視の質が良いと、正確で丁寧な報告が迅速できる
8. 障害の内容と対応方法を糧として調整する
   - 監視の見直し（閾値とかメトリックの内容とか）
   - 改善するための知見が得られる
     - **アプリケーションの成長に繋がる**
     - 自動復旧（リトライ処理とか）
     - レスポンスタイムの測定
       - **改善したタイミングでも通知を行う事が大事**
     - CDNが障害を起こした際にオリジンを見るようにフォールバックする
     - キューの監視
     - TimerTriggerが未発火だった場合の対処
       - 毎次実行させる
     - FunctionRuntimeの突然死
       - ハートビート検知を行う
9.  ↑までのことを行うと障害の早期発見に繋がる
   - 成長の結果
   - 弊社ではSlackに通知をしている
     - 作りやすいから
       - WebhookでOK
     - メールは見ないからやらない
       - 障害が発生するとメールが数百通とかになる
     - チャンネルで分類できる
       - テナントごとに管理しやすい
