...
Azure AD B2C(以下 AAD B2C) はAzure AD の1サービスです。Azure AD B2C を使うと迅速に認証や認可部分の機能を立ち上げることができます。
...
開発用の SDK も提供されていますが、標準化された認証プロトコルを使用しているため、様々なアプリケーションや製品と統合することが出来ます。

ユーザ情報を AAD B2C に統合することも可能ですが、ユーザの詳細な情報を Dynamics や Salesforce といった CRM や外部の DB などと統合することも可能です。

価格に関しても、今日後の Auth0 や Cognito などと比較しても比較的低価格で提供されているため、導入コストも比較的低く済みます。(価格については、後述)
...
自動で作成されるものなので、よくわからないから削除するといったことをしてはいけません。
...
まず、認証基盤を自前で作った場合に考えられる危険であったり大変な事を考えてみましょう。

* センシティブな情報を直接扱う必要がある(顧客情報の詰まった DB を直接管理する必要がある)
* 強い認証方式を用意する必要がある
* 実装ミスによって顧客のパスワードが流出した
  - パスワードが平文で保存されていた
...
例えば3つのサービスを作り、認証基盤を統一したい場合といった場合どのように実装したらよいでしょうか。
...
ということは、わざわざ認証基盤として使うアプリケーションをもう一つ用意する必要が発生します。
...
認証基盤は、サービスの要です。
この部分が貧弱だと、サインイン・サインアップが滞るなどユーザ体験を著しく損なう可能性があります。
...
AAD B2C は、名前の通り B to C を想定したサービスです。
そのため、所謂会員管理が必要な EC であったり、スマートフォンのアプリのようなサービスをターゲットとしています。

ユーザ登録、ログインなどの基本的な機能や、メール文面のカスタマイズといった部分に関しては、全て独自にカスタマイズが可能なので、大抵のサービスの要件は、満たすことが出来ると思われます。
...
.NET Core のアプリケーションに OIDC サーバの機能を追加するためのミドルウェアです。
Identity Server を使用すれば、自由にカスタマイズの出来る OIDC サーバを構築することが可能です。

これを使用する場合、自由な実装が可能ですが、インフラの管理やなどの非機能要件を検討する必要があるなど、実装・運用双方にコストが掛かります。

(Identity Server のリンクなど)
...
Web アプリケーションの認証基盤として、AAD B2C を使用
...
AADB2Cで基本的な認証を行った後で、外部のデータストアでユーザー情報を補完する構成です。

B2C は、フローの途中で REST API を呼び出すことが可能なので、B2C で保持していないプラスアルファの情報を、Salesforce などの CRM から取得し、
レスポンスの内容に含めることが出来ます。
この構成を取ることによって、既存の顧客情報を移行することなく B2C のメリットを享受出来ます。

### 構成例3

☆画像と説明してね

B2C は、メールアドレス認証や登録完了時などにメールを送信する部分が存在します。
ですが、デフォルトのメール文面は、内容が簡素であったり、Microsoft から送信されているという旨の記載があったりと
サービスの世界観と合わせることが難しい文面になっています。

そこで、メール送信機能を外部のメールプロバイダに変更して自由にメールの文面をカスタマイズする構成を取る必要があります。
外部のメールプロバイダを使用する場合は、自由にメール文面のカスタマイズが出来るので、サービスの世界観を壊さずにシステムを構築できます。

注意点として、使用できるメールプロバイダは、REST API でメール送信する機能が存在するものに限ります。
